...
> If you are using a Rasperry Pi with The Homebridge image flashed to your SD Card you must change the line `ExecStart=/usr/bin/npm start` to `ExecStart=npm start` as the Homebridge image changes the path of `npm`. 
